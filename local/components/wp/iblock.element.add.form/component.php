...
if ($bAllowAccess)
{
	...
	elseif ($USER->GetID())
	{
		...
		// �����. �������� �� ����������� ������������ � ����������� ��������, ���� �� �� ���������� $arFilter["CREATED_BY"] = $arFilds['CREATED_BY']
		$res = CIBlockElement::GetByID($arParams["ID"]);
		if ($ar_res = $res->GetNextElement()) {
			$arFilds = $ar_res->GetFields();
		}
		$usercompanyid = getUserField ('USER', $USER->GetID(), 'UF_ID_COMPANY');
		$res = CIBlockElement::GetByID($usercompanyid);
		if ($uc_res = $res->GetNextElement()) {
			$usProps = $uc_res->GetProperties();
		}
		if (in_array($USER->GetID(), $usProps['staff']['VALUE'])) {
			$arFilter["CREATED_BY"] = $arFilds['CREATED_BY'];
		} else {
			$arFilter["CREATED_BY"] = $USER->GetID();
		}
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
		{
			...
			if (intval($propertyID) > 0)
			{
				...
				else
				{
					...
					if ($propertyID == PROPERTY_ID_IMAGES_IN_GALLERY_PHOTO) {
						...
						foreach ($arPropertyValue as $key => $value)
						{
							if (strpos($key, 'n') !== false) {
								...
								$newkey = substr($key, 1);
								$arFile = array('name' => $value['name'], 'type' => $value['type'], 'tmp_name' => $_SERVER['DOCUMENT_ROOT'].'/upload/tmp'.$value['tmp_name'],'size' => $value['size'], 'error' => $value['error']);
								$arUpdatePropertyValues[$propertyID][$newkey] = $arFile;									
							} else {
								if ($_REQUEST["PROPERTY_del"][$propertyID][$key] == "Y") {
									$IB = 11;
									$ID = $arParams['ID'];
									$CODE = 'images';
									$arProp = CIBlockElement::GetProperty($IB, $ID, 'ID', 'DESC', array('CODE' => $CODE));
									while($ar_props = $arProp->Fetch())
										{
											if ($value == $ar_props['VALUE']) {
												CIBlockElement::SetPropertyValueCode($ID, $CODE, array($ar_props['PROPERTY_VALUE_ID'] => array('del' => 'Y')));
					...
					elseif ($propertyID == PROPERTY_ID_ADD_PHOTO_IN_CATALOG) {
						...
						foreach ($arPropertyValue as $key => $value)
						{
							if (strpos($key, 'n') !== false) {
								...
								if ($_REQUEST["PROPERTY_del"][$propertyID][$key] == "Y") {
									$IB = IBLOCK_ID_CATALOG;
									$ID = $arParams['ID'];
									$CODE = 'additionalPhoto';
									$arProp = CIBlockElement::GetProperty($IB, $ID, 'ID', 'DESC', array('CODE' => $CODE));
									while($ar_props = $arProp->Fetch())
										{
											if ($value == $ar_props['VALUE']) {
					...
					//pre($arUpdatePropertyValues, true);
			...
			else
			{
				...
				else
				{
					if($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "F")
					{
						if ($_FILES["PROPERTY_FILE_".$propertyID."_0"]["error"] == 4) { // Сработает при копировании материала 
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			$sRedirectUrl .= "&elementId=".urlencode($arParams["ID"]);
			$sRedirectUrl .= "&iBlockId=".urlencode($arParams["IBLOCK_ID"]);
