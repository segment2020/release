...
if (!empty($arResult["ERRORS"]))
{
	...
	header('Location: ' . $location . 'edit/?errorStr=' . $errorStr . '&elementId=' . $arParams['ID'] . '&iBlockId=' . $arParams['IBLOCK_ID']);
	exit();
}
...
{ 
	...
	elseif ($arResult["MESSAGE"] === 'Элемент успешно добавлен')
	{
		header('Location: ' . $location . '?iBlockId=' . $_GET['iBlockId'] . '&msg=OK');
		exit();
	}
}
?>
