...
<div class="block-title clearfix">
	<a class="notitlestyle" href="<?php echo $arResult['SECTION_PAGE_URL']; ?>"><?php echo $arResult['NAME']; ?></a><a class="floatright" href="<?php echo $arResult['SECTION_PAGE_URL']; ?>">Все обзоры<i class="icon-icons_main-10"></i></a>
</div>
<div class="row"> 
<?php
foreach ($arResult["ITEMS"] as $itemkey => $arItem) {
    ...
    if ('1' === $arItem["PROPERTIES"]["showLogo"]['VALUE']) {
        ...
        if ($fileNum) {
            $file = CFile::ResizeImageGet($fileNum, array('width'=>320, 'height'=>160), BX_RESIZE_IMAGE_PROPORTIONAL, true);
        }
    } elseif ($arItem["PREVIEW_PICTURE"]["SRC"]) {
        $file = CFile::ResizeImageGet($arItem["PREVIEW_PICTURE"]["ID"], array('width'=>320, 'height'=>160), BX_RESIZE_IMAGE_PROPORTIONAL, true);
    }

    if (!isset($file)) {
        $file['src'] = '';
    } ?>   
	<div class="col-sm-6 col-xs-6 cell-12-xs <?php if ($itemkey==0) {
        echo 'leftimg';
    } else {
        echo 'rightimg';
		...
		<div class="mb_background">
			...
			<div class="infotvc">
				<span class="infotime"><?php echo $dateCreate; ?></span>
				<span class="infoview"><i class="icon-icons_main-05"></i><?php echo showviews($arItem['ID']); ?></span>
				<span class="infocomment"><i class="icon-icons_main-04"></i><?php echo $msgCounter; ?></span>
			</div>
		</div>
	</div>
<?
