<div class="col-xs-12">
        ...
        <div class="row">
            <div id="author-pick" class="clearfix col-xs-3">
                ...
                <select onchange="toggleAuthorType()" class="selectpicker selectboxbtn form-control minbr" id="toggleAuthor" name="PROPERTY[<?= $fromCompanyId ?>][0]">
                    <option value="" <? if (empty($fromCompanyValue)) { ?> selected <? } ?>>Компании
                    </option>
                    <option value="<?= $listFromCompanyValue ?>" <? if (!empty($fromCompanyValue)) { ?> selected <? } ?>>Автора
                    </option>
                </select>
            </div>
    ...
    ?> <div>
            ...
            <!-- перенос материала -->
            <input type="hidden" name="PROPERTY[<?= $companyToId ?>][0]" value="<?= $arFields['ID'] ?>">
        </div>
    <? } ?>
    <? if (CSite::InGroup(array(1))) { ?>
        <div class="block-moveTo clearfix">Поместить материал в:
            <select class="selectpicker selectboxbtn form-control minbr" data-live-search="true" id="moveTo" name="PROPERTY[<?= $moveToId ?>][0]">
                <option value="<?= IBLOCK_ID_ALL_MATERIALS ?>" <? if ($moveToValue == IBLOCK_ID_ALL_MATERIALS) { ?> selected <? } ?>>Без категории
                </option>
                <option value="<?= IBLOCK_ID_NEWS_COMPANY ?>" <? if ($moveToValue == IBLOCK_ID_NEWS_COMPANY) { ?> selected <? } ?>>Новости компании
                </option>
                <option value="<?= IBLOCK_ID_NEWS_INDUSTRY ?>" <? if ($moveToValue == IBLOCK_ID_NEWS_INDUSTRY) { ?> selected <? } ?>>Новости отрасли
                </option>
                <? if (CSite::InGroup(array(9))) { ?>
                    <option value="<?= IBLOCK_ID_LIFE_INDUSTRY ?>" <? if ($moveToValue == IBLOCK_ID_LIFE_INDUSTRY) { ?> selected <? } ?>>Редакционные статьи
                    </option>
                <? } ?>
                <option value="<?= IBLOCK_ID_VIEWPOINT ?>" <? if ($moveToValue == IBLOCK_ID_VIEWPOINT) { ?> selected <? } ?>>Мнения
                </option>
                <option value="<?= IBLOCK_ID_PRODUCTS_REVIEW ?>" <? if ($moveToValue == IBLOCK_ID_PRODUCTS_REVIEW) { ?> selected <? } ?>>Товарные обзоры
                </option>
                <option value="<?= IBLOCK_ID_STOCK ?>" <? if ($moveToValue == IBLOCK_ID_STOCK) { ?> selected <? } ?>>Акции
                </option>
                <option value="<?= IBLOCK_ID_NOVETLY ?>" <? if ($moveToValue == IBLOCK_ID_NOVETLY) { ?> selected <? } ?>>Новинки
                </option>
            </select>
        </div>
